...
# API đề xuất
@app.get("/recommend")
def recommend_api(user_id: int = Query(None), course_name: str = Query(None), alpha: float = 0.5):
    if user_id is None and course_name is None:
        return {"error": "Please provide user_id or course_name."}
    ...
    if user_id is not None:
        ...
        cf_predictions = [svd.predict(user_id, cid) for cid in course_ids]
        cf_scores = {pred.iid: pred.est for pred in cf_predictions}

        if course_name is None:
            top_courses = sorted(cf_scores.items(), key=lambda x: x[1], reverse=True)[:6]
            for course_id, _ in top_courses:
                course_name = courses[courses['course_id'] == course_id]['course_name'].iloc[0]
                recommended_courses.append(course_name)
        else:
            try:
                course_index = courses[courses['course_name'] == course_name].index[0]
                course_id = courses.iloc[course_index]['course_id']
                distances = similarity[course_index]
                content_scores = sorted(list(enumerate(distances)), reverse=True, key=lambda x: x[1])[1:10]
                content_scores = {courses.iloc[idx]['course_id']: score for idx, score in content_scores}
                hybrid_scores = {
                    cid: alpha * content_scores[cid] + (1 - alpha) * cf_scores.get(cid, 0)
                    for cid in content_scores
                }
                top_courses = sorted(hybrid_scores.items(), key=lambda x: x[1], reverse=True)[:6]
                for course_id, _ in top_courses:
        ...
        try:
            course_index = courses[courses['course_name'] == course_name].index[0]
            distances = sorted(list(enumerate(similarity[course_index])), reverse=True, key=lambda x: x[1])[1:6]
            for i in distances:
                course_name = courses.iloc[i[0]].course_name
                recommended_courses.append(course_name)
        except IndexError:
            return {"error": f"Course '{course_name}' not found."}
