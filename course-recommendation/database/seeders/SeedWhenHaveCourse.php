...
class SeedWhenHaveCourse extends Seeder
{
    public function run(): void
    {
        // Seed Users - Create 50 diverse users (students, admins, instructors)
        $this->seedUsers();
        
        // Seed Categories - Create course categories
        $this->seedCategories();
        
        // Seed Course Categories - Link existing courses to categories
        $this->seedCourseCategories();
        
        // Seed Students - Create student records for student users
        $this->seedStudents();
        
        // Seed Student Categories - Link students to categories
        $this->seedStudentCategories();
        
        // Seed Admins - Create admin records
        $this->seedAdmins();
        
        // Seed Admin Accounts - Create financial accounts for admins
        $this->seedAdminAccounts();
        
        // Seed Instructors - Create instructor records
        $this->seedInstructors();
        
        // Seed Instructor Accounts - Create financial accounts for instructors
        $this->seedInstructorAccounts();
        
        // Seed Instructor Requests - Create instructor application requests
        $this->seedInstructorRequests();
        
        // Seed Course Instructors - Assign instructors to existing courses
        $this->seedCourseInstructors();
        
        // Seed Course Reviews - Create admin reviews for courses
        $this->seedCourseReviews();
        
        // Seed Interactions - Create user-course interactions
        $this->seedInteractions();
        
        // Seed Enrollments - Create course enrollments
        $this->seedEnrollments();
        
        // Seed Certificates - Issue certificates for completed courses
        $this->seedCertificates();
        
        // Seed Coupons - Create discount coupons
        $this->seedCoupons();
        
        // Seed Payments - Record course payments
        $this->seedPayments();
        
        // Seed Revenue Sessions - Create revenue periods
        $this->seedRevenueSessions();
        
        // Seed Revenue Distributions - Distribute revenue to instructors
        $this->seedRevenueDistributions();
        
        // Seed Lessons - Create lessons for existing courses
        $this->seedLessons();
        
        // Seed Lesson Progress - Track student progress in lessons
        $this->seedLessonProgress();
        
        // Seed Quizzes - Create quizzes for lessons
        $this->seedQuizzes();
        
        // Seed Questions - Create quiz questions
        $this->seedQuestions();
        
        // Seed Question Choices - Create answer choices for questions
        $this->seedQuestionChoices();
        
        // Seed Quiz Results - Record quiz results
        $this->seedQuizResults();
        
        // Seed User Answers - Record individual question answers
        $this->seedUserAnswers();
        
        // Seed Reviews - Create user reviews for courses
        $this->seedReviews();
        
        // Seed Forum Posts - Create discussion posts
        $this->seedForumPosts();
        
        // Seed Media - (Commented out, but updated to work with existing courses)
        // $this->seedMedia();
        
        // Seed Similarity Matrix - Create course similarity scores
    ...
    private function seedCourseCategories()
    {
        ...
        foreach ($courses as $course) {
            // Skip if course already has categories
            if (CourseCategory::where('course_id', $course->id)->exists()) {
                continue;
            }
    ...
    private function seedStudentCategories()
    {
        ...
        foreach ($students as $student) {
            // Skip if student already has categories
            if (StudentCategory::where('student_id', $student->id)->exists()) {
                continue;
            }
    ...
    private function seedAdmins()
    {
        ...
        foreach ($adminUsers as $adminUser) {
            if (!Admins::where('user_id', $adminUser->id)->exists()) {
                Admins::create([
    ...
    private function seedAdminAccounts()
    {
        ...
        foreach ($admins as $admin) {
            if (!AdminAccount::where('admin_id', $admin->id)->exists()) {
                AdminAccount::create([
                    'admin_id' => $admin->id,
    ...
    private function seedInstructors()
    {
        ...
        foreach ($instructorUsers as $user) {
            if (!Instructors::where('user_id', $user->id)->exists()) {
                Instructors::create([
                    'user_id' => $user->id,
    ...
    private function seedInstructorAccounts()
    {
        ...
        foreach ($instructors as $instructor) {
            if (!InstructorAccount::where('instructor_id', $instructor->id)->exists()) {
    ...
    private function seedInstructorRequests()
    {
        ...
        foreach ($students as $student) {
            if (!InstructorRequest::where('user_id', $student->id)->exists()) {
                $status = $statuses[array_rand($statuses)];
                $admin = $admins->isNotEmpty() && $status !== 'pending' ? $admins->random() : null;
                
                InstructorRequest::create([
                    'user_id' => $student->id,
                    'name' => 'Instructor Applicant ' . $student->username,
    ...
    private function seedCourseReviews()
    {
        ...
        $courses = Course::where('status', 'pending')->inRandomOrder()->take(min(20, Course::count()))->get();
        $admins = Admins::all();
        $statuses = ['approved', 'rejected'];
        
        if ($admins->isEmpty() || $courses->isEmpty()) {
            $this->command->warn('No admins or pending courses found. Skipping course_reviews seeding.');
            return;
        }
        
        foreach ($courses as $course) {
            if (!CourseReview::where('course_id', $course->id)->exists()) {
                CourseReview::create([
                    'course_id' => $course->id,
                    'admin_id' => $admins->random()->id,
                    'status' => $statuses[array_rand($statuses)],
                    'notes' => 'Review notes for course ' . $course->course_name,
    ...
    private function seedInteractions()
    {
        ...
        if ($students->isEmpty() || $courses->isEmpty()) {
            $this->command->warn('No students or courses found. Skipping interactions seeding.');
            return;
        }
        
        foreach ($students as $student) {
            // Each student interacts with 1-5 courses
            $interactionCount = rand(1, min(5, $courses->count()));
            $selectedCourses = $courses->random($interactionCount);
            
            foreach ($selectedCourses as $course) {
                if (Interaction::where('user_id', $student->id)->where('course_id', $course->id)->exists()) {
                    continue;
                }
    ...
    private function seedPayments()
    {
        ...
        $enrollments = Enrollment::inRandomOrder()->take(min(30, Enrollment::count()))->get();
        $methods = ['momo', 'zalopay', 'bank_transfer', 'vnpay'];
        $coupons = Coupon::all();
        
        foreach ($enrollments as $enrollment) {
            if (!Payment::where('user_id', $enrollment->user_id)
                ->where('course_id', $enrollment->course_id)
                ->exists()) {
                $couponId = rand(0, 100) < 20 && $coupons->isNotEmpty() ? $coupons->random()->id : null;
                
                // Use course price if available, else fallback to random
                $course = Course::find($enrollment->course_id);
                $amount = $course && $course->price ? $course->price : rand(50000, 500000);
    ...
    private function seedRevenueSessions()
    {
        ...
        foreach ($months as $month) {
            if (!RevenueSession::where('month', $month)->where('year', $year)->exists()) {
    ...
    private function seedRevenueDistributions()
    {
        ...
        $courseInstructors = Course_Instructors::inRandomOrder()->take(min(20, Course_Instructors::count()))->get();
        $statuses = ['pending', 'completed', 'failed'];
        
        if ($revenueSessions->isEmpty() || $courseInstructors->isEmpty()) {
            $this->command->warn('No revenue sessions or course instructors found. Skipping revenue_distributions seeding.');
            return;
        }
        
        foreach ($courseInstructors as $courseInstructor) {
            if (!RevenueDistribution::where('revenue_session_id', $revenueSessions->random()->id)
                ->where('instructor_id', $courseInstructor->instructor_id)
                ->where('course_id', $courseInstructor->course_id)
                ->exists()) {
    ...
    private function seedLessons()
    {
        ...
        foreach ($courses as $course) {
            ...
            for ($i = 1; $i <= $lessonCount; $i++) {
                Lesson::create([
                    'course_id' => $course->id,
                    'title' => "Lesson $i: Topic for {$course->course_name}",
    ...
    private function seedQuizzes()
    {
        ...
        foreach ($lessons as $lesson) {
            ...
            for ($i = 1; $i <= $quizCount; $i++) {
                Quiz::create([
                    'lesson_id' => $lesson->id,
                    'title' => "Quiz $i for Lesson {$lesson->title}",
                    'max_attempts' => 3,
    ...
    private function seedQuestions()
    {
        ...
        foreach ($quizzes as $quiz) {
            if (Question::where('quiz_id', $quiz->id)->exists()) {
                continue;
            }
            ...
            for ($i = 1; $i <= $questionCount; $i++) {
                Question::create([
                    'quiz_id' => $quiz->id,
                    'title' => "Question $i for Quiz {$quiz->title}",
    ...
    private function seedQuestionChoices()
    {
        ...
        foreach ($questions as $question) {
            if (QuestionChoice::where('question_id', $question->id)->exists()) {
                continue;
            }
            ...
            for ($i = 1; $i <= $choiceCount; $i++) {
                ...
                QuestionChoice::create([
                    ...
                    'content' => $question->question_type === 'true_false' 
                        ? ($i === 1 ? 'True' : 'False') 
                        : "Choice $i for Question {$question->title}",
    ...
    private function seedQuizResults()
    {
        ...
        foreach ($lessonProgresses as $progress) {
            ...
            foreach ($quizzes as $quiz) {
                if (!QuizResult::where('user_id', $progress->user_id)
    ...
    private function seedUserAnswers()
    {
        ...
        foreach ($quizResults as $quizResult) {
            ...
            foreach ($questions as $question) {
                if (!UserAnswer::where('user_id', $quizResult->user_id)
        ...
        foreach ($interactions as $interaction) {
            if (!Review::where('user_id', $interaction->user_id)
                ...
                Review::create([
                    ...
                    'feedback_type' => 'content_quality', // Default feedback type
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);
            }
    ...
    private function seedForumPosts()
    {
        ...
        foreach ($interactions as $interaction) {
            ...
            for ($i = 1; $i <= $postCount; $i++) {
                if (!ForumPost::where('user_id', $interaction->user_id)
                    ->where('course_id', $interaction->course_id)
                    ->where('title', "Discussion Topic $i")
                    ->exists()) {
                    ForumPost::create([
                        'user_id' => $interaction->user_id,
                        'course_id' => $interaction->course_id,
    ...
    private function seedSimilarityMatrix()
    {
        ...
        if ($courses->count() < 2) {
            $this->command->warn('Not enough courses to create similarity matrix. Skipping.');
            return;
        }
        ...
        foreach ($courses as $course1) {
            foreach ($courses as $course2) {
                if ($course1->id < $course2->id) { // Avoid duplicates and self-similarity
                    if (!SimilarityMatrix::where('course_id_1', $course1->id)
                        ->where('course_id_2', $course2->id)
                        ->exists()) {
